...
const Navbar = () => {
  ...
  // RESTORED: Icon size 18 (Bada size)
  const navItems = [
  ...
  return (
    <>
      ...
      >
        {/* RESTORED: Large Container Padding */}
        <div className="w-full px-4 xs:px-6 lg:px-10">
          
          {/* RESTORED: Large Header Height (h-[140px]) */}
          <div className="flex justify-between items-center h-24 md:h-[140px] transition-all duration-300">
            ...
            <div className="flex-shrink-0 flex items-center">
              <Link to="/" className="flex items-center gap-3 group">
                ...
                {/* RESTORED: Big Logo Size (h-[230px]) */}
                <img 
                  src="/durablelogo.png" 
                  alt="Durable Fasteners" 
                  className="h-16 xs:h-18 md:h-[170px] w-auto object-contain transition-transform duration-300" 
                />
                
                {/* Separator Line */}
                <div className="h-12 xs:h-14 md:h-20 w-px bg-gray-300 mx-2"></div>
                
                {/* RESTORED: Big Classone Logo (h-[210px]) */}
                <img 
                  src="/classone.png" 
                  alt="Classone" 
                  className="h-14 xs:h-16 md:h-[170px] w-auto object-contain mt-1"
            ...
            <div className="hidden xl:flex flex-1 justify-center items-center gap-2">
              ...
              {/* Dynamic Menu Items with RESTORED Padding (px-4 py-3) */}
              {navItems.map((item) => (
      ...
      {/* MOBILE DRAWER (Keeping the Side Drawer because it's better for responsiveness, but adjusted for big header) */}
      ...
      <div className={`xl:hidden fixed top-0 right-0 h-full w-[85%] max-w-[320px] bg-white z-50 shadow-2xl transition-transform duration-300 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
